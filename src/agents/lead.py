...
class LeadAgent:
    ...
    def _load_history(self, user_id: int):
        ...
        # list comprehension
        return [types.Content(role=row["role"], parts=[types.Part(text=row["message_text"])])for row in response.data]
...
# jelaskan kegunaan dari variabel self.tool pada function handle_send_message
